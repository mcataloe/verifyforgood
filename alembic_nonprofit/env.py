...
from verification.backend.shared.platform import resolve_nonprofit_postgres_sqlalchemy_url  # noqa: E402
from verification.backend.shared.customer_accounts import CustomerAccountsBase  # noqa: E402
from verification.backend.shared.nonprofits import NONPROFIT_TABLE_NAMES  # noqa: E402
...
def _resolve_sqlalchemy_url() -> str:
    ...
    if configured:
        return configured
    return resolve_nonprofit_postgres_sqlalchemy_url(os.environ)


def _include_object(object_, name, type_, reflected, compare_to):  # noqa: ANN001
    del reflected, compare_to
    if type_ == "table":
        return name in NONPROFIT_TABLE_NAMES
    table = getattr(object_, "table", None)
    if table is not None:
        return table.name in NONPROFIT_TABLE_NAMES
    return True
...
def run_migrations_offline() -> None:
    ...
    context.configure(
        ...
        version_table="alembic_version_nonprofit",
    )
