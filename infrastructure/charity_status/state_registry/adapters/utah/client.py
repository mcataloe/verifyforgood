...
class UtahRegistryClient:
    ...
    def search(self, *, normalized_name: str) -> list[RawStateRegistryRecord]:
        ...
        return _parse_utah_search_results(html)

    def _request_html(self, normalized_name: str) -> str:
        params = urllib.parse.urlencode({"entityName": normalized_name})
        ...
        try:
            with urllib.request.urlopen(request, timeout=self._timeout_seconds) as response:
                if response.status >= 400:
                    raise StateRegistryError(f"Utah registry request failed with status {response.status}")
                return response.read().decode("utf-8", errors="ignore")
        except StateRegistryError:
            raise
        except Exception as exc:
            raise StateRegistryError(f"Utah registry request failed: {exc}") from exc


def _parse_utah_search_results(html: str) -> list[RawStateRegistryRecord]:
    records = []
        ...
        records.append(
            {
                ...
                "entity_type": row.get("Type").text if row.get("Type") else None,
                "formation_date": row.get("Registration Date").text if row.get("Registration Date") else None,
            }
        )
