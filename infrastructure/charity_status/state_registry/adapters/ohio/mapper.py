...
PARSER_VERSION = "ohio_business_registry.v1"
SOURCE_NAME = "ohio_secretary_of_state"
...
def map_ohio_record(
    ...
    if standing in {None, StateRegistryStanding.UNKNOWN} and normalize_entity_name(raw_record.get("status")) in {"CANCELLED", "DEAD", "HOLD", "MERGED"}:
...
def _map_status(value: object | None):
    ...
    if normalized in {"CANCELLED", "HOLD"}:
