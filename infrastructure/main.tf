locals {
  ...
  standardized_resource_names = {
    ...
    api_ecr_repository                = "${local.namespace}-${local.platform}-api-${local.environment_slug}-${local.region_short}"
    api_ecs_service                   = "${local.namespace}-${local.platform}-api-service-${local.environment_slug}-${local.region_short}"
    api_task_definition               = "${local.namespace}-${local.platform}-api-task-${local.environment_slug}-${local.region_short}"
    api_task_execution_role           = "${local.namespace}-${local.platform}-api-task-execution-${local.environment_slug}-${local.region_short}"
    api_task_role                     = "${local.namespace}-${local.platform}-api-task-${local.environment_slug}-${local.region_short}"
    api_task_security_group           = "${local.namespace}-${local.platform}-api-task-sg-${local.environment_slug}-${local.region_short}"
    api_alb_security_group            = "${local.namespace}-${local.platform}-api-alb-sg-${local.environment_slug}-${local.region_short}"
    api_log_group                     = "${local.namespace}-${local.platform}-api-log-group-${local.environment_slug}-${local.region_short}"
    api_alb                           = substr("${local.namespace}-${local.platform}-api-alb-${local.environment_slug}-${local.region_short}", 0, 32)
    api_target_group                  = substr("${local.namespace}-${local.platform}-api-tg-${local.environment_slug}-${local.region_short}", 0, 32)
    worker_ecr_repository             = "${local.namespace}-${local.platform}-worker-${local.environment_slug}-${local.region_short}"
    worker_ecs_service                = "${local.namespace}-${local.platform}-worker-service-${local.environment_slug}-${local.region_short}"
    worker_task_definition            = "${local.namespace}-${local.platform}-worker-task-${local.environment_slug}-${local.region_short}"
    worker_task_execution_role        = "${local.namespace}-${local.platform}-worker-task-execution-${local.environment_slug}-${local.region_short}"
    worker_task_role                  = "${local.namespace}-${local.platform}-worker-task-${local.environment_slug}-${local.region_short}"
    worker_task_security_group        = "${local.namespace}-${local.platform}-worker-task-sg-${local.environment_slug}-${local.region_short}"
    worker_log_group                  = "${local.namespace}-${local.platform}-worker-log-group-${local.environment_slug}-${local.region_short}"
  ...
  legacy_resource_names = {
    ...
    api_ecr_repository                = "${local.legacy_name_prefix}-api"
    api_ecs_service                   = "${local.legacy_name_prefix}-api-service"
    api_task_definition               = "${local.legacy_name_prefix}-api-task"
    api_task_execution_role           = "${local.legacy_name_prefix}-api-task-execution"
    api_task_role                     = "${local.legacy_name_prefix}-api-task"
    api_task_security_group           = "${local.legacy_name_prefix}-api-task-sg"
    api_alb_security_group            = "${local.legacy_name_prefix}-api-alb-sg"
    api_log_group                     = "${local.legacy_name_prefix}-api-log-group"
    api_alb                           = substr("${local.legacy_name_prefix}-api-alb", 0, 32)
    api_target_group                  = substr("${local.legacy_name_prefix}-api-tg", 0, 32)
    worker_ecr_repository             = "${local.legacy_name_prefix}-worker"
    worker_ecs_service                = "${local.legacy_name_prefix}-worker-service"
    worker_task_definition            = "${local.legacy_name_prefix}-worker-task"
    worker_task_execution_role        = "${local.legacy_name_prefix}-worker-task-execution"
    worker_task_role                  = "${local.legacy_name_prefix}-worker-task"
    worker_task_security_group        = "${local.legacy_name_prefix}-worker-task-sg"
    worker_log_group                  = "${local.legacy_name_prefix}-worker-log-group"
  ...
  api_ecr_repository_name                = local.resource_names.api_ecr_repository
  api_ecs_service_name                   = local.resource_names.api_ecs_service
  api_task_definition_family             = local.resource_names.api_task_definition
  api_task_execution_role_name           = local.resource_names.api_task_execution_role
  api_task_role_name                     = local.resource_names.api_task_role
  api_task_security_group_name           = local.resource_names.api_task_security_group
  api_alb_security_group_name            = local.resource_names.api_alb_security_group
  api_log_group_name                     = local.resource_names.api_log_group
  api_alb_name                           = local.resource_names.api_alb
  api_target_group_name                  = local.resource_names.api_target_group
  worker_ecr_repository_name             = local.resource_names.worker_ecr_repository
  worker_ecs_service_name                = local.resource_names.worker_ecs_service
  worker_task_definition_family          = local.resource_names.worker_task_definition
  worker_task_execution_role_name        = local.resource_names.worker_task_execution_role
  worker_task_role_name                  = local.resource_names.worker_task_role
  worker_task_security_group_name        = local.resource_names.worker_task_security_group
  worker_log_group_name                  = local.resource_names.worker_log_group
