...
| `GAP-006` | Customer decision authority not documented at initial Recon | Charter and Decision Authority now explicitly state customer decision ownership | Documentation gap is closed; runtime compatibility labels remain | `INIT-001` for runtime/contract follow-up | Resolved as documentation gap |
...
| `GAP-013` | Root README owns too many truths | README combines architecture, API, operations, history, and product copy and still lacks the planned authority notice | Stale details can compete with canonical docs | Apply bounded root README authority patch with safe local patching | Open |
...
| `GAP-023` | Large root/source-truth files still need safe entry-point edits | `AGENTS.md`, `README.md`, and `TODO.md` contain stale ownership/topology wording and are large existing files | Agents may continue to prefer old README-first source truth or stale topology until bounded root patches land | Apply exact bounded edits with a local checkout or a tool that can patch in place; do not reconstruct the full files through connector-only replacement | Open / blocks BU-GOV-007 completion |
