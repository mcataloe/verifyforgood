"""FastAPI application ownership for the backend platform API runtime."""
...
from verification.backend.customer.api.runtime import handle_api_event
...
async def _dispatch_runtime_request(request: Request, *, resource: str) -> Response:
    ...
    return runtime_response_to_http(handle_api_event(event))


def create_app() -> FastAPI:
    app = FastAPI(title="VerifyForGood Platform API", version="1.0.0")
    ...
    def register_route(resource_spec) -> None:
        ...
        endpoint_name = (
            f"platform_{'_'.join(methods).lower()}_"
