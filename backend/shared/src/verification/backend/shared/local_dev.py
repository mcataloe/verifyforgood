...
for _path in (
    _REPO_ROOT / "backend" / "customer-api" / "src",
    _REPO_ROOT / "backend" / "platform-api" / "src",
    _REPO_ROOT / "backend" / "worker" / "src",
    _REPO_ROOT / "backend" / "shared" / "src",
    _REPO_ROOT / "backend" / "ingest" / "federal" / "src",
    _REPO_ROOT / "backend" / "ingest" / "state" / "src",
    _REPO_ROOT / "backend" / "ingest" / "shared" / "src",
):
    if _path.exists():
...
from verification.backend.shared.runtime.nonprofit_db_cutover import cutover_nonprofit_database
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="verification.backend.shared.local_dev")
    parser.add_argument(
        "command",
        choices=(
            "db-upgrade",
