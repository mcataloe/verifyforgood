...
from verification.backend.shared.auth.models import AuthenticatedPrincipal
from verification.backend.shared.billing import EntitlementService
from verification.backend.shared.billing.service import DEFAULT_PLANS, check_quota_and_calculate, monthly_period_for
from verification.backend.shared.core.models import AuthContext
from verification.backend.shared.customer_accounts import usage_metrics_for_route
...
class OAuthClientCredentialsService:
    ...
    def issue_token(self, client_id: str, client_secret: str) -> tuple[AuthContext, dict[str, Any]]:
        ...
        from verification.backend.shared.auth.oauth import issue_client_access_token
...
def load_oauth_token_store(raw_json: str) -> StaticOAuthTokenStore:
    ...
    records = []
    from verification.backend.shared.auth.oauth import StoredOAuthTokenRecord
...
def load_oauth_client_store(raw_json: str) -> StaticOAuthClientStore:
    ...
    records = []
    from verification.backend.shared.auth.oauth import StoredOAuthClientRecord
...
def _from_context(auth_context: AuthContext) -> AuthenticatedPrincipal:
    from verification.backend.shared.auth.models import AuthenticatedPrincipal, ApiPlan
