...
def interpret_jurisdiction_compliance(enrichment: dict[str, Any] | None) -> dict[str, Any]:
    ...
    for provider in providers:
        ...
        if not isinstance(fields, dict):
            continue
        if not any(key in fields for key in JURISDICTION_COMPLIANCE_KEYS):
            continue
...
COMPLIANCE_KEYS = JURISDICTION_COMPLIANCE_KEYS
extract_state_compliance = interpret_jurisdiction_compliance
