...
from charity_status.platform import DEFAULT_APP_NAME, DEFAULT_PUBLIC_BRAND_NAME
...
def test_load_branding_config_defaults_to_capability_based_names():
    ...
    assert config.public_brand_name == "Verification Platform"
    assert config.user_agent() == "verification-platform/1.0"
    assert DEFAULT_APP_NAME == "verification-platform"
    assert DEFAULT_PUBLIC_BRAND_NAME == "Verification Platform"
    ...
    config = load_branding_config(
        {
            ...
            "PUBLIC_BRAND_NAME": "Verify for Good",
        }
    )

    assert config.app_name == "Verify For Good Runtime"
    assert config.public_brand_name == "Verify for Good"
...
def test_http_stripe_checkout_client_uses_configured_public_brand_name():
    client = HttpStripeCheckoutClient(secret_key="sk_test_123", public_brand_name="Verify for Good")
    ...
    assert captured["payload"]["description"] == "Verify for Good account acct_123"
...
def test_infrastructure_wires_runtime_branding_configuration():
    ...
    assert "PUBLIC_BRAND_NAME=Verification Platform" in readme
