...
def test_api_dockerfile_uses_asgi_entrypoint():
    ...
    assert "PYTHONPATH=/app:/app/infrastructure:/app/private-platform/src:/app/backend/api/src:/app/backend/shared/src" in dockerfile
...
def test_worker_and_ingest_dockerfiles_use_backend_runtime_entrypoints():
    ...
    assert 'ENTRYPOINT ["python", "-m", "charity_status_backend.ingest_task.cli"]' in ingest
