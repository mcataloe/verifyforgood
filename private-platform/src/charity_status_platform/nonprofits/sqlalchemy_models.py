...
class NonprofitModel(CustomerAccountsBase):
    ...
    filings: Mapped[list["NonprofitFilingModel"]] = relationship(back_populates="nonprofit", cascade="all, delete-orphan")
    sources: Mapped[list["NonprofitSourceModel"]] = relationship(back_populates="nonprofit", cascade="all, delete-orphan")
...
class NonprofitSourceModel(CustomerAccountsBase):
    ...
    __table_args__ = (
        ...
        Index("ix_nonprofit_sources_nonprofit_retrieved", "nonprofit_id", "retrieved_at"),
    ...
    required_for_eligibility: Mapped[bool | None] = mapped_column(Boolean, nullable=True)
    ...
    source_signature: Mapped[str | None] = mapped_column(String(128), nullable=True)
    normalized_data: Mapped[dict[str, Any] | None] = mapped_column(JSON_VARIANT, nullable=True)
    raw_payload: Mapped[dict[str, Any] | None] = mapped_column(JSON_VARIANT, nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
...
class ComplianceCheckModel(CustomerAccountsBase):
    ...
    registration_jurisdiction: Mapped[str | None] = mapped_column(String(64), nullable=True)
    registration_expiration_date: Mapped[date | None] = mapped_column(Date, nullable=True)
    ...
    state_business_good_standing: Mapped[bool | None] = mapped_column(Boolean, nullable=True)
    final_recommendation: Mapped[str | None] = mapped_column(String(64), nullable=True)
    flags_json: Mapped[dict[str, Any] | list[Any] | None] = mapped_column(JSON_VARIANT, nullable=True)
    reasons_json: Mapped[dict[str, Any] | list[Any] | None] = mapped_column(JSON_VARIANT, nullable=True)
    evidence_json: Mapped[dict[str, Any] | list[Any] | None] = mapped_column(JSON_VARIANT, nullable=True)
    summary_json: Mapped[dict[str, Any] | None] = mapped_column(JSON_VARIANT, nullable=True)
    metadata_json: Mapped[dict[str, Any] | None] = mapped_column("metadata", JSON_VARIANT, nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
