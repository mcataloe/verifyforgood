...
- may depend on other `verification_platform` service areas
- deploy-time code may invoke this layer, not the reverse

Current transition helpers:

- `backend_contracts.py`
  - canonical private-platform import root for API response-envelope and route-version helpers that still live under `verification.api`
- `api_compat.py`
